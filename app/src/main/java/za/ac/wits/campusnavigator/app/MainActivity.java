 ...
 */
public final class MainActivity extends AppCompatActivity implements HasGetBuildingsUseCase {

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(za.ac.wits.campusnavigator.app.R.layout.activity_main);

        if (savedInstanceState == null) {
            showFragment(new MapFragment());
        }

        BottomNavigationView bottomNav = findViewById(za.ac.wits.campusnavigator.app.R.id.bottomNav);
        bottomNav.setOnItemSelectedListener(item -> {
            int id = item.getItemId();
            if (id == za.ac.wits.campusnavigator.app.R.id.nav_map) {
                showFragment(new MapFragment());
                return true;
            } else if (id == za.ac.wits.campusnavigator.app.R.id.nav_common_picks
                    || id == za.ac.wits.campusnavigator.app.R.id.nav_favourites
                    || id == za.ac.wits.campusnavigator.app.R.id.nav_settings) {
    ...
    private void showFragment(@NonNull Fragment fragment) {
        getSupportFragmentManager()
                .beginTransaction()
                .replace(za.ac.wits.campusnavigator.app.R.id.fragmentContainer, fragment)
                .commit();
    }
